...
describe("utils", () => {
  describe("#getEtherscanLink", () => {
    it("correct for tx", () => {
      expect(getMaticExplorerLink(1, "abc", "transaction")).toEqual(
        "https://maticexplorer.com/tx/abc"
      );
    });
    it("correct for token", () => {
      expect(getMaticExplorerLink(1, "abc", "token")).toEqual(
        "https://maticexplorer.com/token/abc"
      );
    });
    it("correct for address", () => {
      expect(getMaticExplorerLink(1, "abc", "address")).toEqual(
        "https://maticexplorer.com/address/abc"
      );
    });
    it("unrecognized chain id defaults to mainnet", () => {
      expect(getMaticExplorerLink(2, "abc", "address")).toEqual(
        "https://maticexplorer.com/address/abc"
      );
    });
    it("ropsten", () => {
  ...
  describe("#basisPointsToPercent", () => {
    it("converts basis points numbers to percents", () => {
      ...
      expect(
        basisPointsToPercent(110.00000000000001).equalTo(
          new Percent(JSBI.BigInt(110), JSBI.BigInt(10000))
        )
      ).toBeTruthy();
    });
  });
