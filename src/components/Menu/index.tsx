...
import { Menu as UikitMenu, connectorLocalStorageKey, ConnectorNames } from "cryption-uikit";
...
const Menu = (props) => {
  ...
  useEffect(() => {
    ...
    if (getTrxItme) {
      ...
      if (getTrxItme[accountId] !== null || getTrxItme[accountId] !== undefined) {
  ...
  return (
    <UikitMenu
      ...
      login={['/swap', '/find', '/pool', '/add'].includes(location.pathname) ? loginEther : login}
      logout={['/swap', '/find', '/pool', '/add'].includes(location.pathname) ? logoutEther : logout}
      logoIcon={LogoIcon}
      isDark={isDark}
